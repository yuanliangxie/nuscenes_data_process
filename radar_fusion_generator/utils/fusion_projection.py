...
from nuscenes.utils.data_classes import PointCloud,RadarPointCloud
from radar_fusion_generator.utils import radar
...
def _create_vertical_line_x(P1, P2, img):
    """
    the fuction is be construct by xie, because the code writed by author is a shit!
    :param P1:
    :param P2:
    :param img:
    :return:
    ...
    create_coordinate_bottom = [P1[0], P1[1]]
    create_coordinate_up = [P1[0], P1[1] - dYa]

    if P1[0] > 0 and P1[0] < imageW and P1[1] < imageH and (P1[1] - dYa) > 0:
        return [P1[0], P1[1], dYa]
    elif P1[0] > 0 and P1[0] < imageW and P1[1] < imageH and P1[1] > 0:
        create_coordinate_up[1] = 0
        return [P1[0], P1[1], np.abs(P1[1] - 0)]
    else:
        return None, None, None

def _filter_point_x(P1, img):
    """
    the fuction is be construct by xie, because the code writed by author is a shit!
    :param P1:
    :param P2:
    ...
    if P1[0] > 0 and P1[0] < imageW and P1[1] < imageH and P1[1] > 0:
        return [P1[0], P1[1]]
    else:
        return None, None

def _create_circle_x(P1, P2, img):
    """
    the fuction is be construct by xie, because the code writed by author is a shit!
    :param P1:
    :param P2:
    :param img:
    :return:[circle_center_coordinates,radius]
    """
    # define local variables for readability
    imageH = img.shape[0]
    imageW = img.shape[1]
    ...
    if P1[0] > 0 and P1[0] < imageW and P1[1] > 0 and P1[1] < imageH:
...
def _scale_mask_point(image_data, radar_data, radar_xyz_endpoints):
    no_of_points = radar_data.shape[1]
    mask = np.zeros(no_of_points)
    for radar_point in range(0, no_of_points):
        _, _, dYa = _create_vertical_line_x(radar_data[0:2, radar_point], radar_xyz_endpoints[0:2, radar_point], image_data)
        if dYa == None:
            continue
        elif dYa > 141:
            mask[radar_point]=1
...
def _radar2camera_line(image_data, radar_data, radar_xyz_endpoints, clear_radar=False):
    """
    ...
    if clear_radar:
        pass  # we just don't add radar_data to the image_plus_channel
    else:
        for radar_point in range(0, no_of_points):
            x, y_bottom, dYa = _create_vertical_line_x(
                radar_data[0:2, radar_point], radar_xyz_endpoints[0:2, radar_point], image_data)
            if x == None:
                continue
            x, y_bottom = int(x), int(y_bottom)
            temp_radar_line = radar_extension[y_bottom-dYa:y_bottom+1, x]
            expand_radar_point_data =np.tile(radar_data[3:, radar_point ].reshape(1, -1), (dYa+1,1))
            mask1 = expand_radar_point_data[:, -3] < temp_radar_line[:, -3]#distance这个属性在倒数第3行，优选距离近的，但是在检测的话应该优选距离远的，因为近的好检测，这里小于号是优选距离远的
            #小于号距离近的优先，大于号是距离远的优先
            mask2 = ~np.any(temp_radar_line, axis=1)
            mask = mask1|mask2
            temp_radar_line[mask, :] = expand_radar_point_data[mask, :]
            radar_extension[y_bottom-dYa:y_bottom+1, x, :] = temp_radar_line
    image_plus = np.concatenate((image_data, radar_extension), axis=2)

    return image_plus

def _radar2camera_circle(image_data, radar_data, radar_xyz_R, clear_radar=False):
    """
        ...
        :param radar_data: [numpy array (xyz+meta x no of points)] that consists of the transformed radar points with z = 0
            default semantics: x y z dyn_prop id rcs vx vy vx_comp vy_comp is_quality_valid ambig_state x_rms y_rms invalid_state pdh0 vx_rms vy_rms distance
        :param radar_xyz_R: [numpy array (3 x no of points)] that consits of the transformed radar points z = R
        :param clear_radar: [boolean] True if radar data should be all zero
    ...
    if clear_radar:
        ...
        for radar_point in range(0, no_of_points):
            x, y, radius = _create_circle_x(radar_data[0:2, radar_point], radar_xyz_R[0:2, radar_point], image_data)
            if x == None:
                continue
            radius = int(radius)
            x, y = int(x), int(y)
            x1 = int(np.clip(x-radius, 0, np.inf))
            y1 = int(np.clip(y-radius, 0, np.inf))
            x2 = int(np.clip(x+radius, 0, image_data.shape[1]-1))
            y2 = int(np.clip(y+radius, 0, image_data.shape[0]-1))
            dx = int(x2 - x1)
            dy = int(y2 - y1)

            #此程序先通过方框框住含有圆的区域，再通过圆的方程获得mask，然后根据mask赋值

            temp_radar_square = radar_extension[y1:y2+1, x1:x2+1]
            expand_radar_point_data =np.tile(radar_data[3:, radar_point ], (dy+1, dx+1, 1))
            mask1 = expand_radar_point_data[:, :, -3] < temp_radar_square[:, :, -3]#distance这个属性在倒数第3行，优选距离近的，但是在检测的话应该优选距离远的，因为近的好检测
            mask2 = ~np.any(temp_radar_square, axis=2)
            #下面对圆所在的边界情况进行了处理，因为圆的边界被下边截断是else情况，正常，且雷达映射在
            #图像上边时，又小且不可能，因为上边是天空，所以忽略考虑。
            if x1== 0:#圆在左边被截断
                x_coor = np.tile(np.arange(2*radius-dx, 2*radius+1), (dy + 1, 1)) - int(radius)
                y_coor = np.tile(np.arange(2*radius-dy, 2*radius+1), (dx+1, 1)).transpose(1, 0) - int(radius)
            else:#圆在正常区域，或者被图像右边截断
                x_coor = np.tile(np.arange(dx+1), (dy + 1, 1)) - int(radius)
                y_coor = np.tile(np.arange(dy+1), (dx + 1, 1)).transpose(1, 0) - int(radius)

            mask3 = (x_coor**2 + y_coor**2) <= radius**2#通过计算距离来得出圆的mask
            mask = (mask1|mask2)&mask3#(空白地方或者距离更近的点)并上（在圆内的点）
            temp_radar_square[mask, :] = expand_radar_point_data[mask, :]
            radar_extension[y1:y2+1, x1:x2+1, :] = temp_radar_square

    image_plus = np.concatenate((image_data, radar_extension), axis=2)

    return image_plus

def _radar2camera_point(image_data, radar_data, clear_radar=False):
    """
    ...
    if clear_radar:
        ...
        for radar_point in range(0, no_of_points):
            x, y = _filter_point_x(radar_data[0:2, radar_point], image_data)
            if x == None:
                continue
            x, y = int(x), int(y)
            temp_radar_point = radar_extension[y, x].reshape(1, -1)
            expand_radar_point_data = radar_data[3:, radar_point ].reshape(1, -1)
            mask1 = expand_radar_point_data[:, -3] < temp_radar_point[:, -3]#distance这个属性在倒数第3行，优选距离近的，但是在检测的话应该优选距离远的，因为近的好检测
            mask2 = ~np.any(temp_radar_point, axis=1)
            mask = mask1|mask2
            temp_radar_point[mask, :] = expand_radar_point_data[mask, :]
            radar_extension[y, x, :] = temp_radar_point
    ...
    if clear_radar:
        ...
        for radar_point in range(0, no_of_points):
            ...
            if projection_line.shape[0]==0:
                ...
                continue
            radar_point_valid +=1
            print('雷达有效个数：', radar_point_valid)
            print("y:", projection_line[0, 1])
...
def imageplus_creation(nusc, image_data, radar_data, pointsensor_token, camera_token, \
                       image_target_shape=(900, 1600), clear_radar=False, clear_image=False, radar_cover_shape="line"):
    """
    ...
    radar_points_bottom, radar_points_up = _radar_transformation(radar_data, (0, 2.5))#这是对点进行分层
    radar_xyz_bottom = map_pointcloud_to_image(nusc, radar_points_bottom.copy(), pointsensor_token=pointsensor_token,
                                           camera_token=camera_token, target_resolution=image_target_shape)
    radar_xyz_up = map_pointcloud_to_image(nusc, radar_points_up.copy(), pointsensor_token=pointsensor_token,
                                                 camera_token=camera_token, target_resolution=image_target_shape)
    mask = _scale_mask_point(cur_img, radar_xyz_bottom, radar_xyz_up)

    #get the radarpoint depend on the radar_conver_shape
    if radar_cover_shape == "normal_point":
        radar_points, _ = _radar_transformation(radar_data, (0.8, 0.8))  # 正常的雷达点,因为雷达点的高度为0.5,这里选两个点是为了兼容，但只取一个点
        radar_points = map_pointcloud_to_image(nusc, radar_points, pointsensor_token=pointsensor_token,
                                               camera_token=camera_token, target_resolution=image_target_shape)
        image_plus = _radar2camera_point(cur_img, radar_points, clear_radar=clear_radar)
        large_scale_image_plus = _radar2camera_point(cur_img, radar_points[:, mask == 1], clear_radar=clear_radar)
        middle_scale_image_plus = _radar2camera_point(cur_img, radar_points[:, mask == 2], clear_radar=clear_radar)
        little_scale_image_plus = _radar2camera_point(cur_img, radar_points[:, mask == 3], clear_radar=clear_radar)

    elif radar_cover_shape == "line":
        radar_points_bottom1, radar_points_up1 = _radar_transformation(radar_data, (0, 2.5))
        radar_xyz_bottom1 = map_pointcloud_to_image(nusc, radar_points_bottom1, pointsensor_token=pointsensor_token,
                                               camera_token=camera_token, target_resolution=image_target_shape)

        radar_xyz_up1 = map_pointcloud_to_image(nusc, radar_points_up1, pointsensor_token=pointsensor_token,
                                                     camera_token=camera_token, target_resolution=image_target_shape)
        image_plus = _radar2camera_line(cur_img, radar_xyz_bottom1, radar_xyz_up1, clear_radar=clear_radar)
        large_scale_image_plus = _radar2camera_line(cur_img, radar_xyz_bottom1[:, mask == 1],
                                                    radar_xyz_up1[:, mask == 1], clear_radar=clear_radar)
        middle_scale_image_plus = _radar2camera_line(cur_img, radar_xyz_bottom1[:, mask==2],
                                                     radar_xyz_up1[:, mask==2], clear_radar=clear_radar)
        little_scale_image_plus = _radar2camera_line(cur_img, radar_xyz_bottom1[:, mask==3],
                                                     radar_xyz_up1[:, mask==3], clear_radar=clear_radar)


        # radar_points, radar_xyz_endpoint = _radar_transformation(radar_data, (0, 2.5))
        # radar_points = map_pointcloud_to_image(nusc, radar_points, pointsensor_token=pointsensor_token,
        #                                        camera_token=camera_token, target_resolution=image_target_shape)
        #
        # radar_xyz_endpoint = map_pointcloud_to_image(nusc, radar_xyz_endpoint, pointsensor_token=pointsensor_token,
        #                                              camera_token=camera_token, target_resolution=image_target_shape)
        # image_plus = _radar2camera_line(cur_img, radar_points, radar_xyz_endpoint, clear_radar=clear_radar)

        # large_scale_image_plus = _radar2camera_line(cur_img, radar_points[:, mask==1], radar_xyz_endpoint[:, mask==1], clear_radar=clear_radar)
        # middle_scale_image_plus = _radar2camera_line(cur_img, radar_points[:, mask==2],
        #                                              radar_xyz_endpoint[:, mask==2], clear_radar=clear_radar)
        # little_scale_image_plus = _radar2camera_line(cur_img, radar_points[:, mask==3],
        #                                              radar_xyz_endpoint[:, mask==3], clear_radar=clear_radar)

    elif radar_cover_shape == "circle":
        radar_points, radar_xyz_R = _radar_transformation(radar_data, (0.8, 1.3))#R半径为0.5米，1-0.5=0.5
        radar_points = map_pointcloud_to_image(nusc, radar_points, pointsensor_token=pointsensor_token,
                                               camera_token=camera_token, target_resolution=image_target_shape)
        radar_xyz_R = map_pointcloud_to_image(nusc, radar_xyz_R, pointsensor_token=pointsensor_token,
                                                     camera_token=camera_token, target_resolution=image_target_shape)
        image_plus = _radar2camera_circle(cur_img, radar_points, radar_xyz_R, clear_radar=clear_radar)

        large_scale_image_plus = _radar2camera_circle(cur_img, radar_points[:, mask == 1],
                                                      radar_xyz_R[:, mask == 1], clear_radar=clear_radar)
        middle_scale_image_plus = _radar2camera_circle(cur_img, radar_points[:, mask == 2],
                                                       radar_xyz_R[:, mask ==2], clear_radar=clear_radar)
        little_scale_image_plus = _radar2camera_circle(cur_img, radar_points[:, mask == 3],
                                                       radar_xyz_R[:, mask ==3], clear_radar=clear_radar)

    else:
        raise ValueError('There is no type of cover_shape you want')
    ...
    #image_plus = _radar2camera_x(cur_img, radar_points, radar_xyz_endpoint, clear_radar=clear_radar)
    ...
    return image_plus, large_scale_image_plus, middle_scale_image_plus, little_scale_image_plus

# def generate_mask(nusc, cur_img, radar_data, image_target_shape, pointsensor_token, camera_token):
#     radar_points_bottom, radar_xyz_up = _radar_transformation(radar_data, (0, 2))  # 这是对点进行分层
#     radar_points_bottom = map_pointcloud_to_image(nusc, radar_points_bottom, pointsensor_token=pointsensor_token,
#                                                   camera_token=camera_token, target_resolution=image_target_shape)
#     radar_xyz_up = map_pointcloud_to_image(nusc, radar_xyz_up, pointsensor_token=pointsensor_token,
#                                            camera_token=camera_token, target_resolution=image_target_shape)
#     mask = _scale_mask_point(cur_img, radar_points_bottom, radar_xyz_up)
#     return mask
...
def create_imagep_visualization(image_plus_data, color_channel="distance", \
        draw_circles=False, radar_lines_opacity=0.6):
    """
    ...
    if n_channels > 3:
        ...
        radar_color = np.array([173, 255, 47]).astype(np.uint8)#绿黄色#np.array([255, 0, 0]).astype(np.uint8)#渲染红色
            ...
            for y in range(0, image_plus_height):
                ...
                for pixel in [(y, x)]:  # [(y,x-1),(y,x),(y,x+1)]:
                    if image_data.shape > pixel:
                        ...
                        pixel_color = np.squeeze(
                            cv2.addWeighted(pixel_color, 1 - radar_lines_opacity, radar_color, radar_lines_opacity, 0))
                ...
                if draw_circles:
                    if image_plus_data.shape[0] > y + 1 and not np.any(image_plus_data[y + 1, x, 3:]):  # 在最下面画圆
                        cv2.circle(image_data, (x, y), 3, color=radar_color.astype(np.float), thickness=1)
...
if __name__ == '__main__':
    ...
    from radar_fusion_generator.utils.nuscenes_helper import get_sensor_sample_data

    # execute functions
    # Initialize the database
    home =  os.path.expanduser("~")
    nusc = NuScenes(version='v1.0-mini', dataroot='/home/xyl/桌面/v1.0-mini', verbose=True)
    ...
    image_plus_data = imageplus_creation(nusc,
        image_data, radar_data, radar_token, camera_token, image_target_shape, clear_radar=False, clear_image=False, radar_cover_shape="normal_point")

 
    # Visualize the result
    imgp_viz = create_imagep_visualization(image_plus_data,draw_circles=True)
